...
//#define SPI_SENDER_SYNC

#ifdef SPI_SENDER_SYNC
static HAL_StatusTypeDef SPIS_send_sync(const TestCommand_t* const command) {
	const HAL_StatusTypeDef status = HAL_SPI_Transmit(SPI_SENDER, (uint8_t*) command->pattern, command->header.pattern_len, HAL_MAX_DELAY);
	SPI_RECEIVER->Instance->CR1 |= SPI_CR1_SSI; // turn OFF
	if (status != HAL_OK) {
		printf("Failed to sendIT on SPI sender: %d, error 0x%"PRIX32"\r\n", status, HAL_SPI_GetError(SPI_SENDER));
	}
	return status;
}
#endif
...
#ifdef TEST_LOOPBACK
static HAL_StatusTypeDef SPIS_perform_loopback(const TestCommand_t* const command) {
	spis_state_tmp = spis_state & SPI_TEST_MASK;
	HAL_StatusTypeDef status = HAL_SPI_TransmitReceive(SPI_RECEIVER, (uint8_t*) command->pattern, (uint8_t*) spi_buffer, command->header.pattern_len, HAL_MAX_DELAY);
	if (status != HAL_OK) {
		printf("Failed to receiveIT on SPI receiver: %d, error 0x%"PRIX32"\r\n", status, HAL_SPI_GetError(SPI_SENDER));
		spis_state |= SPI_SENDER_ERR | SPI_RECEIVER_ERR;
	}
	else {
		spis_state |= SPI_RECEIVER_RXED;
	}
...
#ifdef TEST_LOOPBACK
		// test peripheral itself
		return SPIS_perform_loopback(command) == HAL_OK ? RC_TEST_IN_PROGRESS : RC_ERROR;
#else
		SPI_RECEIVER->Instance->CR1 |= SPI_CR1_SSI; // turn OFF
		HAL_StatusTypeDef status = HAL_SPI_Receive_IT(SPI_RECEIVER, (uint8_t*) spi_buffer, command->header.pattern_len);
		if (status != HAL_OK) {
...
#ifdef SPI_SENDER_SYNC
		return SPIS_send_sync(command) == HAL_OK ? RC_TEST_IN_PROGRESS : RC_ERROR;
#else
		status = HAL_SPI_Transmit_IT(SPI_SENDER, (uint8_t*) command->pattern, command->header.pattern_len);
		if (status != HAL_OK) {
			SPI_RECEIVER->Instance->CR1 |= SPI_CR1_SSI; // turn OFF
			printf("Failed to sendIT on SPI sender: %d, error 0x%"PRIX32"\r\n", status, HAL_SPI_GetError(SPI_SENDER));
			return RC_ERROR;
		}
#endif // SPI_SENDER_SYNC
#endif // TEST_LOOPBACK
	}
	// check if IRQ happened
