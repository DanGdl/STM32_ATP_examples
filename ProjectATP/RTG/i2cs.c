...
TestResult_t I2CS_perform_test(const TestCommand_t* const command, uint32_t* const test_id) {
	...
	*test_id = i2c_cmd.header.test_id;
	if (i2c_test_status == 0) {
		...
		HAL_StatusTypeDef status = HAL_I2C_Slave_Receive_IT(I2C_RECEIVER, (uint8_t*) i2c_buffer, command->header.pattern_len);
		if (status != HAL_OK) {
		...
		status = HAL_I2C_Master_Transmit_IT(I2C_SENDER, I2C_RECEIVER->Init.OwnAddress1, (uint8_t*) command->pattern, command->header.pattern_len);
		if (status != HAL_OK) {
