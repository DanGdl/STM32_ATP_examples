...
TestResult_t UARTS_perform_test(const TestCommand_t* const command, uint32_t* const test_id) {
	...
	*test_id = uart_cmd.header.test_id;
	if (uart_test_status == 0) {
		...
		HAL_StatusTypeDef status = HAL_UART_Receive_IT(UART_RECEIVER, (uint8_t*) uarts_buffer, command->header.pattern_len);
		if (status != HAL_OK) {
		...
		status = HAL_UART_Transmit_IT(UART_SENDER, command->pattern, command->header.pattern_len);
		if (status != HAL_OK) {
